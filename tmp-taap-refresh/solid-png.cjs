...
async function solidPng({ src, dest, bg, pad = PAD, paintCream = false }) {
  ...
  console.log(dest, `${om.width}x${om.height}`, `${(fs.statSync(out).size / 1024).toFixed(0)}kb`);
  return om;
}
...
(async () => {
  ...
  await solidPng({ src: "intro.png", dest: "intro-plane.png", bg: CREAM });
  await solidPng({ src: "origins.png", dest: "origins.png", bg: CREAM });
  await solidPng({ src: "gen1.png", dest: "gen1-table.png", bg: CREAM });
  await solidPng({
    src: "gen2-panel.png",
    dest: "gen2-screens.png",
    bg: NAVY,
    paintCream: true,
  });
  await solidPng({
    src: "handoff-panel.png",
    dest: "handoff-desktop.png",
    bg: NAVY,
    paintCream: true,
  });
  await solidPng({ src: "gen3.png", dest: "gen3-annotated.png", bg: CREAM });
})();
